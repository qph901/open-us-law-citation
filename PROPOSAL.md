# Proposal: Open US Law Citation

## Summary

Build **Open US Law Citation**, a foundation for reliable legal references: a supported citation should identify the intended provision, retrieve faithful text from a specified version, and retain auditable source evidence. Missing, ambiguous, stale, or unverified references must remain explicit.

The current scope is a **broad, measurable, date-pinned law-coverage and retrieval system** over the `vaquill/open-us-law` dataset (snapshot **v2026.08**), commissioned federally against edition-pinned USLM/GovInfo for USC and point-in-time eCFR for CFR. Coverage measures how broadly the system can deliver reliable citations: which applicable law is represented accurately, which is missing or stale, and what can be retrieved safely. Citation detection, parsing, normalization, and resolution are required correctness capabilities. Retrieval optimization comes only after correctness is measured against official denominators. The project name expresses this purpose; the existing milestone order, acceptance criteria, and deferred scope remain in effect.
...
Combined the collision-provenance and segment-order spikes into one experiment, **evidence-based, not an upstream-ETL trace**. Report at `reports/M0.5A1_segment_provenance.md`, built by `open_us_law_citation.segment_provenance` (DuckDB `file_row_number` + memory-limited spill over the 11 GB federal `text` column).
...
| `variant_capture` (whole-group superset) | 327 | return the member containing **all** others | `partial` until corroborated |
| `variant_capture` (no whole-group superset) | 49 | no member subsumes the group | `ambiguous` |
...
contains **every** other member's, returning that member is provably never partial *relative to
...
Built in `src/open_us_law_citation/citation_parser.py` (reports `reports/M2_parse_selfcheck.md`
...
- **Stage A (detection)** measured as precision/recall over a **hand-labelled gold set** (36
  passages, 34 citations, 11 adversarial distractors — dates, `$` amounts, `Rule 12(b)(6)`, Public
  Law / Fed. Reg. numbers, version strings, phone numbers, and qualified-shaped non-citations):
  **precision 1.000, recall 1.000, F1 1.000** for USC, CFR, and combined. Precision-first: zero
  false positives on any distractor. The gold set caught a real free-text bug (the CFR section
  truncated `240.10b-5` → `240.1` under `finditer`), now fixed and regression-tested. Handled:
  ABSOLUTE citations; enumerated `§§ a, b` lists (each member emitted under the shared title, with a
  negative-lookahead guard so a list running into a *new* citation `…, and 5 U.S.C. § 552` never
  mis-attributes it — only a plural sign licenses the continuation); and the qualified prose form
  (`section 1983 of title 42, United States Code`), which fires **only** when the spelled-out code
  name is present, so `section 5 of title I of the Act` never does. Baseline thresholds recorded as
  empirical targets. Deferred: full corpus-scale in-body detection over the `text` column (overlaps M4).
