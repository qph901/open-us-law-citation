...
_Generated by `open_us_law_citation.identity_collisions`. `text` aggregates run in DuckDB with a hard memory limit + disk spill so the 11 GB federal-regulations `text` column never materializes a whole row-group._
